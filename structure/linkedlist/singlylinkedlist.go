...
type Singly[T any] struct {
	length int
	// Note: here the Node contains both the next and previous node,
	// but the Singly methods use only the next node.
	Head *Node[T]
}
...
// DelAtBeg removes the node at the head (beginning)
// of the list and returns its value.
...
// DelAtEnd removes the node at the end of
...
// DelByPos removes a node in the middle based on
// its position in the list and returns its value.
// Returns false if the list is empty or its length is
// no longer than the specified position.
